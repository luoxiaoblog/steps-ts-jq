...
class Step {
  ...
  createElement(): any {
    ...
    obj.find('.lyj-step__title').text(this.title)
       .next().text(this.description)
    if (this.icon === undefined) {
      obj.find('.lyj-step__icon').addClass('is-text')
    } else {
      obj.find('.lyj-step__icon').addClass('is-icon')
         .find('.lyj-step__icon-inner').addClass(this.icon)
    }
    return obj
  }
}
...
class Steps {
  ...
  refleshStepStatus(el: any) {
    ...
    this.stepList.forEach((step: Step, i: number) => {
      let statusObj = step.element.find('.lyj-step__head, .lyj-step__title, .lyj-step__description')
      if (i < this.active) {
        statusObj.removeClass('is-' + this.processStatus)
                 .removeClass('is-wait')
        ...
        statusObj.removeClass('is-' + this.finishStatus)
                 .removeClass('is-wait')
